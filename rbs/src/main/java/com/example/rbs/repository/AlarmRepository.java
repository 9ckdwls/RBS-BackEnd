...
public interface AlarmRepository extends JpaRepository<Alarm, Integer> {
	...
	// 미해결된 모든 알람
	@Query("SELECT a FROM Alarm a " + "WHERE ( " + "(a.userId = :userId OR a.targetUserId = :userId) "
			+ "OR (a.role = :role OR a.role = 'ROLE_ALL') " + ") " + "AND a.resolved = 'UNRESOLVED' "
			+ "AND NOT ( :role = 'ROLE_ADMIN' AND a.type LIKE '%_CONFIRMED' )")
	...
	// 관리자가 볼 알람
	// 권한이 ROLE_ADMIN or ROLE_ALL 이고 미해결된 알람 or 설치/제거 요청
	@Query("""
			SELECT a FROM Alarm a
						   WHERE (a.resolved = :status AND a.role IN :roles) OR a.type IN :types
						""")
	List<Alarm> findAdminAlarms(@Param("status") AlarmStatus status, @Param("roles") List<String> roles,
	...
	@Query("""
			    SELECT a
			     ...
			     WHERE a.boxId   = :boxId
			       ...
			       AND a.type    LIKE 'COLLECTION%'
			""")
	List<Alarm> findUnresolvedCollectionAlarms(@Param("boxId") int boxId);

	// 타입 리스트, resolved 상태로 Alarm 조회
	List<Alarm> findByBoxIdAndTypeInAndResolved(int boxId, Collection<AlarmType> types, AlarmStatus resolved);
